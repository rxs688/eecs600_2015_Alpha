// hand_detect_server: a simple action server
...
public:
    HandDetectServer(); //define the body of the constructor outside of class definition
...
// of our class HandDetectServer.  Since this is a class method, we need to tell boost::bind that it is a class member,
...
HandDetectServer::HandDetectServer() :
   as_(nh_, "hand_detect", boost::bind(&HandDetectServer::executeCB, this, _1),false) 
// in the above initialization, we name the server "hand_detect"
//  clients will need to refer to this name to connect with this server
{
    ROS_INFO("in constructor of HandDetectServer...");
...
// <hand_detect_server::demoAction> customizes the simple action server to use our own "action" message 
...
int main(int argc, char** argv) {
    ...
    HandDetectServer as_object; // create an instance of the class "HandDetectServer"
    
    ROS_INFO("going into spin");
    // from here, all the work is done in the action server, with the interesting stuff done within "executeCB()"
    // you will see 5 new topics under hand_detect: cancel, feedback, goal, result, status
