...
int main(int argc, char** argv) {
    ...
    while (ros::ok())
    {
        if (cwru_pcl_utils.got_selected_points())
        {
            ...
            ROS_INFO_STREAM("unit normal: " << plane_normal.transpose() << "; dist = " << plane_dist);
            cwru_pcl_utils.fit_points_to_plane (myindices, plane_normal, plane_dist);
            cwru_pcl_utils.copy_indices_pts(myindices);
            ...
            ROS_INFO("Displaying the new set of points to RVIZ");
            ...
            pcl2_display_cloud.header.frame_id = "torso"; //update the time stamp, so rviz does not complain        
