...
{ 
    ROS_INFO("in constructor of ArmMotionInterface, waiting for server: ");
}
...
{    
    ROS_INFO("requesting a joint-space motion plan");
    cart_goal_.command_code =         
    cwru_action::cwru_baxter_cart_moveGoal::RT_ARM_PLAN_JSPACE_PATH_CURRENT_TO_QGOAL;
...
int ArmMotionCommander::rt_arm_execute_planned_path(void)
{
    ...
    cart_move_action_client_.sendGoal(cart_goal_, boost::bind(&ArmMotionCommander::doneCb_, this, _1, _2));
    finished_before_timeout_ = 
    cart_move_action_client_.waitForResult(ros::Duration(computed_arrival_time_+2.0));
...
int ArmMotionCommander::rt_arm_request_tool_pose_wrt_torso(void)
{
    ...
    cart_move_action_client_.sendGoal(cart_goal_, boost::bind(&ArmMotionCommander::doneCb_, this, _1, _2));
    finished_before_timeout_ = cart_move_action_client_.waitForResult(ros::Duration(20.0));
...
void ArmMotionCommander::initalize()
{   
