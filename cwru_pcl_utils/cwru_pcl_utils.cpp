...
void CwruPclUtils::get_transformed_kinect_points(pcl::PointCloud<pcl::PointXYZ> & outputCloud )
{
...
void CwruPclUtils::get_kinect_clr_pts(pcl::PointCloud<pcl::PointXYZRGB> & outputCloud )
{
    ...
    for (int i = 0; i < npts; ++i)
    {
        outputCloud.points[i].getVector3fMap() = pclKinect_clr_ptr_->points[i].getVector3fMap();
        outputCloud.points[i].r = pclKinect_clr_ptr_->points[i].r;
        outputCloud.points[i].g = pclKinect_clr_ptr_->points[i].g;
        outputCloud.points[i].b = pclKinect_clr_ptr_->points[i].b;
    }
}
